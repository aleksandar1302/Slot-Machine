...
class GameManager {
  ...
  // this method switch between two modes (random and fixed)
  switchMode = () => {
  ...
  // by pressing spin button this method is called
  spin = () => {
  ...
  // to activate and deactivate spin button
  ...
  spinAnimation(result) {
    ...
    setTimeout(() => {
      const scoreSpin = this.calculateScore(this.get5Lines(result));
  ...
  // this method return a random result for 5 reel (used in random mode)
  spinResultRandom() {
    const randomResult = [];
    for (let i = 0; i < this.reelCount; i++) {
      randomResult[i] = {
  ...
  // this method get the 5 lines shown symbol
      ...
      for (let j = 0; j < activeLines.length; j++) {
        if (combines) {
          if (
            ...
            combines.includes(activeLines[j][3]) &&
            combines.includes(activeLines[j][4])
          ) {
  ...
  // on dbclick on score this method called and in fixed mode change score to editable mode
  changeScoreByHand = () => {
    if (this.mode === this.items.modes.random) {
      return;
    } else {
  ...
  // when score changed to editable mode on blur this method is called
  ...
  // this method get the row table and the line of reels and blink the table row and
  ...
  // this method remove red line and blinked row of table
  removeBlinking = () => {
    this.redLine.className = "";
    this.payTable.removeBlinking();
  };

  // this method get the score and update score by add property to current score
  // if force property was true the score will equal of score property (not add)
  ...
  // for calculate score we need the all five line and this method get the result
  // and return five line shown in slot machine
  get5Lines(result) {
    const lines = [[], [], [],[],[]];
    ...
    for (let i = 0; i < result.length; i++) {
      ...
      if (element.line === items.availablePosition[0]) {
        ...
        lines[3][i] = this.reelItmes[calcIndex(index + 3)];
        lines[4][i] = this.reelItmes[calcIndex(index + 4)];
        ...
        lines[3][i] = this.reelItmes[calcIndex(index + 2)];
        lines[4][i] = this.reelItmes[calcIndex(index + 3)];
      } 
      else if (element.line === items.availablePosition[2]) {
        lines[0][i] = this.reelItmes[calcIndex(index - 2)];
        lines[1][i] = this.reelItmes[calcIndex(index - 1)];
        lines[2][i] = element.symbol.name;
        lines[3][i] = this.reelItmes[calcIndex(index + 1)];
        lines[4][i] = this.reelItmes[calcIndex(index + 2)];
        
      } 
      else if (element.line === items.availablePosition[3]) {
        lines[0][i] = this.reelItmes[calcIndex(index - 3)];
        lines[1][i] = this.reelItmes[calcIndex(index - 2)];
        lines[2][i] = this.reelItmes[calcIndex(index - 1)];
        lines[3][i] = element.symbol.name;
        lines[4][i] = this.reelItmes[calcIndex(index + 1)];
      } else {
        lines[0][i] = this.reelItmes[calcIndex(index - 4)];
        lines[1][i] = this.reelItmes[calcIndex(index - 3)];
        lines[2][i] = this.reelItmes[calcIndex(index - 2)];
        lines[3][i] = this.reelItmes[calcIndex(index - 1)];
        lines[4][i] = element.symbol.name;
      }
    }
    return lines;
  }

  // this functon create select operation in modal (in fixed mode to select symbol and line)
...
// this class crete reels in slot machine
class Slot {
...
// this function is effect to update score and get
// the object is element that is going change and the end that is final number and the time for duration update the number
...
// this function get an array and return a random item in that array
var randomItemFromAraay = (items) => {
  return items[Math.floor(Math.random() * items.length)];
};

// this function get an array of option and item and return a index of that object contain the item
